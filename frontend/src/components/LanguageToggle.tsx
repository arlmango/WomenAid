...
export function LanguageToggle() {
  ...
  return (
    <div
      className="inline-flex gap-1 rounded-full border border-line bg-surface-2 p-[3px]"
      role="group"
      ...
      {(["ru", "kz"] as const).map((code) => (
        <button
          ...
          className={`min-h-[30px] rounded-full px-3 text-xs font-semibold uppercase transition-colors ${
            lang === code ? "bg-rose text-white shadow-btn" : "text-ink-soft hover:bg-surface-3"
          }`}
        >
          {code}
        </button>
