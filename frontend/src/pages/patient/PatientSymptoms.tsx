    ...
    <div className="space-y-4">
      ...
      <div className="rounded-card border border-navy/15 bg-surface p-5 shadow-soft">
        ...
        <textarea
          ...
          rows={3}
          className="mb-3 w-full rounded-input border border-navy/20 bg-surface-2 px-3.5 py-2.5 text-sm text-ink focus:border-indigo focus:outline-none focus:ring-3 focus:ring-indigo/15"
        />
        <Button
          type="button"
          loading={submitting}
        ...
        >
          {!submitting && <Send size={16} strokeWidth={2.25} />}
          {t("symptomSubmit")}
        </Button>
        ...
        {lastVerdict && (
          <motion.div
            ...
            className={`mt-3 flex items-start gap-2.5 rounded-input border border-navy/15 p-3 text-sm ${
      ...
      {loading ? (
        ...
        <ul className="space-y-2.5">
          {entries.map((entry) => (
            <li
              key={entry.id}
              className="rounded-card border border-navy/15 bg-surface p-4 shadow-soft"
            >
