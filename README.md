...
|-------|---------|---------|--------|
| **`frontend/`** | **Единственный рабочий сайт.** Вход, регистрация, кабинет пациентки (`/patient/*`) и кабинет врача (`/clinic`) — всё в одном Vite/React SPA | Да, реальный FastAPI | **Основной** |
| `frontend-demo/` | Визуальный прототип на мок-данных (без backend) — для показа, когда backend недоступен | Нет, только мок-данные | Вспомогательный, не для продакшна |

Раньше в репозитории был отдельный `frontend-app/` (лендинг с 3D-сценой +
своя копия регистрации/кабинета пациентки) — он удалён: дублирование одного
и того же backend-контракта в двух приложениях по факту приводило к тому, что
один из них тихо ломался при изменении API (см. историю `/auth/register` в
этом README). Один работающий SPA лучше двух частично работающих.
...
cd frontend
npm install
npm run dev        # http://localhost:5173 (backend должен быть на :8000)
```

Экраны: вход (`/auth`, `POST /auth/login`); регистрация (`/auth/register` —
один экран с реальными полями: имя, дата рождения, регион/телефон
(опционально), логин/пароль, полный текст согласия из `GET
/auth/consent-text`, кнопка «Создать аккаунт» недоступна без галочки
согласия — реальный `POST /auth/register`, авто-вход после успеха); кабинет
пациентки (`/patient/*`); кабинет врача (`/clinic`, только `clinician`/`admin`).

Production-сборка: `npm run build` (выход в `frontend/dist/`).

### `frontend-demo/` — визуальный прототип

Visual-only прототип на мок-данных без backend (`src/lib/mockApi.ts`) — для
показа, когда поднимать backend не нужно/не хочется. Не подключён к реальному
API; не использовать для проверки, что что-то реально работает — для этого
нужен `frontend/`.
...
`tests/e2e_register.py`. Во фронтенде — форма `/auth/register` в `frontend/`
(см. «Фронтенды» выше).
...
**Визуальный стиль:** тёплый кремовый фон, бренд-цвета navy/indigo/pink
(токены `--color-navy`/`--color-indigo`/`--color-pink` и т.д. в
`frontend/src/index.css`), мягкие декоративные градиентные «пятна» на фоне,
закруглённые карточки с тонкой тёмной обводкой вместо glassmorphism.
Легаси-имена утилит (`bg-rose`, `text-lavender-deep`, ...) сохранены и
указывают на актуальную палитру — переименовывать классы по всему коду не
требовалось.
