...
## Три фронтенда — какой сейчас основной

Репозиторий содержит три отдельных фронтенд-приложения. Это сознательное
разделение «для показа» / «что реально работает», не путаница — таблица ниже
актуальна.

| Папка | Что это | Backend | Статус |
|-------|---------|---------|--------|
| **`frontend-app/`** | **Текущий основной сайт.** Лендинг (3D/motion-дизайн) + настоящая регистрация/вход через backend + кабинет пациентки | Да, реальный FastAPI | **Основной** |
| `frontend/` | Кабинет врача (`/clinic`) — десктоп-дашборд триажа. Также имеет свой кабинет пациентки на `/patient/*`, не используется напрямую из `frontend-app/` | Да, реальный FastAPI | Действующий, для клиники |
| `frontend-demo/` | Визуальный прототип на мок-данных (без backend) — более ранняя версия для показа, теперь заменена `frontend-app/` | Нет, только мок-данные | Устарел, оставлен для истории |

Клиницист/админ логинится через `frontend-app/`, но видит экран «это кабинет
врача» со ссылкой на `frontend/` (`/auth`) — кабинет врача не дублируется
во втором приложении.

### `frontend-app/` — основной сайт

Vite + React + TypeScript. Библиотеки: `framer-motion` (motion-дизайн),
`@react-three/fiber` + `@react-three/drei` (3D-сцена на лендинге),
`react-router-dom`, `zustand` (auth state).

```bash
cd frontend-app
npm install
npm run dev        # http://localhost:5174 (backend должен быть на :8000)
```

Переменные окружения (опционально, есть рабочие дефолты для локальной разработки):

| Переменная | По умолчанию | Назначение |
|---|---|---|
| `VITE_BACKEND_URL` | `http://localhost:8000` | Куда dev-прокси `/api/*` шлёт запросы |
| `VITE_CLINIC_APP_URL` | `http://localhost:5173/auth` | Куда вести врача/админа после входа (кабинет в `frontend/`) |

Экраны: лендинг (`/`) с 3D-blob-сценой (lazy-загрузка, `prefers-reduced-motion`
и мобильные/touch-устройства получают статичную SVG-версию той же формы —
никогда не блокирует первую отрисовку и не роняет производительность на
мобильном); многошаговая регистрация (`/register` — анкета → логин/пароль →
согласие с полным текстом из `GET /auth/consent-text` → подтверждение, кнопка
«Создать аккаунт» технически недоступна без галочки согласия); вход (`/login`,
настоящий `POST /auth/login`, понятная ошибка + motion-«shake» при неверном
пароле); кабинет пациентки (`/patient/*` — анимированный радиальный таймлайн
скрининга по реальным данным `GET /monitoring/patients/{id}/schedule` и
дневник симптомов через реальные `GET`/`POST
/monitoring/patients/{id}/symptoms`).

Production-сборка: `npm run build` (выход в `frontend-app/dist/`).

### `frontend-demo/` (устарел)

Visual-only прототип на мок-данных без backend — см. историю репозитория.
Не используйте его как основной сайт; для показа используйте `frontend-app/`,
он подключён к настоящему backend и не «играет роль».
...
| `POST /auth/register`                      | все (публичный) — всегда создаёт `role=patient`, см. ниже |
...
### Самостоятельная регистрация (только пациентки)

`POST /auth/register` — публичный, JSON-тело:

```json
{
  "display_name": "Динара Касымова",
...
`display_name`/`birth_date`/`username`/`password`/`consent` обязательны;
`phone`/`region` — опциональны. Создаёт **новую** запись `Patient`
(имя/дата рождения/телефон/регион) + связанный `User(role=patient)` + первую
запись в `consent_records` (через уже существующую версионируемую систему
согласия — `app.consent.record_consent`/`sync_consent_flag`, см. «Путь к
пилоту» ниже; не отдельная параллельная система) — и сразу возвращает `Token`
(тот же формат, что и `/auth/login`), фронтенд авто-логинит пациентку.

**`consent: false` — это технический гейт, не просто валидация формы**: если
поле не `true`, эндпоинт отвечает `400` и **не создаёт вообще ничего** — ни
`Patient`, ни `User`, ни запись согласия (см. CLAUDE.md: «без активного
согласия — AI-анализ невозможен технически»). Канонический текст согласия —
`GET /auth/consent-text` (публичный, `{"version": "...", "text": "..."}`) —
единый источник, который форма регистрации показывает пациентке и который
`/auth/register` записывает как снимок (snapshot) согласия.

У эндпоинта **нет** поля выбора роли: завести аккаунт врача или админа через
него невозможно — это намеренное ограничение RBAC. Аккаунты `clinician`/`admin`
создаются только через `scripts/create_user.py` (см. выше).

```bash
curl -s -X POST http://127.0.0.1:8000/auth/register \
  -H "Content-Type: application/json" \
  -d '{"display_name":"Тестовая Пациентка","birth_date":"1995-03-14",
       "username":"newpatient","password":"newpatient123","consent":true}'
# -> {"access_token":"eyJhbGci...","token_type":"bearer"}
```

Занятый логин — `400`. Без согласия — `400`. Слишком короткий логин/пароль,
дата рождения в будущем — `422` (валидация на границе системы, см.
`RegisterRequest` в `backend/app/schemas/auth.py`). Проверено e2e в
`tests/e2e_register.py`. Во фронтенде — многошаговая форма `/register` в
`frontend-app/` (см. «Три фронтенда» выше).
...
|------------|--------------------|-------------------------------------------------------------------|
| Вход       | `/auth`            | Логин + регистрация (`/auth/register`, только пациентки), редирект по роли из JWT |
| Врача      | `/clinic/*`        | Десктоп-дашборд с сайдбаром: очередь триажа, ревью с `raw_score`/`confidence` |
| Пациентки  | `/patient/*`       | Mobile-first (375px), нижние tabs, выезжающие bottom-sheets        |

**Визуальный стиль:** светлый минимализм в духе Vercel — закруглённые карточки
(`rounded-2xl`, токен `--radius-card` в `frontend/src/index.css`), мягкие тени
(`shadow-soft`/`shadow-soft-hover`) и glassmorphism (`bg-white/70..90` +
`backdrop-blur-xl`) на хроме — шапках, сайдбаре, модалках, bottom-sheets и
toast-уведомлениях. Фирменная розовая палитра (`--color-rose*`,
`--color-lavender*` и т.д.) не менялась — только форма и плотность UI.
...
**Кабинет врача** (`/clinic/*`): сайдбар слева (бренд, навигация, бейдж
статуса модели, профиль/выход) + таблица очереди с горизонтальным скроллом
(`overflow-x-auto`) справа. Клик по строке открывает модалку ревью с
`raw_score` / `confidence` / `model_status`, формой решения
(`POST .../review/{id}`) и ссылкой на PDF-отчёт. `raw_score`/`confidence`
собраны в отдельный code-split чанк (`ClinicQueue` + `ClinicLayout`) — они
физически не входят в JS-бандл, который скачивает сессия пациентки.
...
.venv/bin/python tests/e2e_register.py # самостоятельная регистрация + технический гейт согласия
```
