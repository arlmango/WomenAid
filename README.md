...
## Демо-версия для презентаций (`frontend-demo/`)

`frontend-demo/` — это **визуальный прототип на мок-данных** для показа
инвесторам, клиникам и жюри: полностью кликабельный, без заглушек, без
реального backend. Все «сетевые» запросы обслуживает слой
`src/lib/mockApi.ts` (с искусственной задержкой, чтобы чувствовался живой
запрос), а данные берутся из `src/data/fixtures.ts` (синтетические пациентки,
симптомы, risk-assessment — совпадают по смыслу со `scripts/seed_demo.py`).

> ⚠️ **Чётко разделяйте две вещи, не путайте их:**
>
> | Папка | Что это | Backend |
> |-------|---------|---------|
> | `frontend-demo/` | Визуальный прототип «как это будет» — **для показа** | Нет, только мок-данные |
> | `frontend/` | Настоящее SPA продукта — **реальная интеграция** | Да, реальный FastAPI |
>
> `frontend-demo/` намеренно НЕ подключён к backend и не должен использоваться
> как продакшн. Реальная интеграция живёт в `frontend/`.
...
Экраны: лендинг (`/`), вход с переключением «Я пациентка / Я врач» (`/login`),
мобильный кабинет пациентки (`/patient/*` — радиальный таймлайн скрининга,
дневник симптомов, загрузка снимка → AI-триаж, записи на приём) и
десктоп-дашборд врача (`/clinic` — очередь триажа, карточка пациентки,
PDF-подобный отчёт). Production-сборка — `npm run build` (выход в
`frontend-demo/dist/`).

**Что в демо сознательно НЕ «залакировано»** (это часть продукта, признак
зрелости, а не недоделанность): дисклеймер «это не диагноз, решение за врачом»
на каждом AI-результате, заметный бейдж статуса модели
(`DEMO · NOT_CLINICALLY_VALIDATED`) и подпись о том, что данные пациенток —
синтетические/демонстрационные.
...
`POST /auth/register` — публичный, JSON-тело `{"username": "...", "password": "..."}`
(логин ≥3 символов, пароль ≥8). Всегда создаёт **новую** пустую запись
`Patient` + связанный `User(role=patient)` и сразу возвращает `Token` (тот же
формат, что и `/auth/login`) — фронтенд автоматически логинит пациентку после
регистрации. У эндпоинта **нет** поля выбора роли: завести аккаунт врача или
админа через него невозможно — это намеренное ограничение RBAC. Аккаунты
`clinician`/`admin` создаются только через `scripts/create_user.py` (см. выше).

```bash
curl -s -X POST http://127.0.0.1:8000/auth/register \
  -H "Content-Type: application/json" \
  -d '{"username":"newpatient","password":"newpatient123"}'
# -> {"access_token":"eyJhbGci...","token_type":"bearer"}
```

Занятый логин — `400`. Слишком короткий логин/пароль — `422` (валидация на
границе системы, см. `RegisterRequest` в `backend/app/schemas/auth.py`).
Во фронтенде форма регистрации — `/auth/register`, ссылка на неё есть на
экране входа (`/auth`).
