...
- **Демо-интерфейс (кабинет врача):** http://localhost:8080
- **Мобильный кабинет пациентки (PWA):** http://localhost:8080/patient-pwa/
...
- `frontend/` отдаётся как статика (кабинет врача на `/`, PWA на `/patient-pwa/`);
- `api/index.py` — Python-serverless-функция, которая монтирует реальный FastAPI
  под `/api`, так что существующие вызовы фронтенда `/api/...` и `/health`
  работают без изменений;
- маршрутизация и сборка функции заданы в `vercel.json` (зависимости — слим-набор
...
Два **отдельных** фронтенда под разную аудиторию — намеренно не объединены в
одно приложение (у них разные сценарии и разные требования к офлайн-доступу):

| Кабинет    | Путь            | Что это                                                        |
|------------|-----------------|----------------------------------------------------------------|
| Врача      | `/`             | `frontend/index.html` — десктоп-ориентированный, очередь триажа |
| Пациентки  | `/patient-pwa/` | `frontend/patient-pwa/` — минимальное PWA для телефона          |

**Кабинет пациентки — PWA** (`frontend/patient-pwa/`):
...
- **Устанавливается на домашний экран.** Android/Chrome — кнопка «Добавить на
  ...
  («Поделиться» → «На экран Домой»), пошаговая подсказка показана прямо в
  кабинете.
- **Service worker (`sw.js`) кэширует только статическую оболочку** —
  HTML/CSS/JS/иконки/manifest, чтобы приложение открывалось офлайн.
  **Медицинские данные офлайн не кэшируются:** всё, что идёт через `/api/*` и
  `/health`, service worker никогда не перехватывает и не пишет в Cache Storage —
  это сознательное решение, чувствительные данные доступны только онлайн.
  Токен авторизации хранится в `sessionStorage` (не переживает закрытие вкладки,
  не пишется на диск).
- **Иконки** генерируются воспроизводимо скриптом
...
- **Secure context:** service worker работает на `https://` и на
  `http://localhost` — Docker-демо на `:8080` подходит. При разработке без
  Docker (`python -m http.server --directory frontend 8080`) статика и установка
  работают, но `/api` и `/health` без nginx-прокси недоступны (как и у основного
  `index.html`) — для полного сценария используйте Docker.
