...
## 2026-06-27 — Убран кастомный курсор + полировка UI (P1+P2+P3 по согласованному плану)

- **Шаг 0 — обычный курсор:** убран `<CustomCursor />` из `App.tsx` (импорт +
  элемент) и правило `cursor: none` (`.custom-cursor-active`) из `index.css`.
  Файл `CustomCursor.tsx` оставлен в репо, но нигде не импортируется (не
  выполняется). `CursorGlow` (мягкое свечение, курсор не прячет) — оставлен.
  Проверка computed-стилей на `/`, `/auth`, `/auth/register`: `body cursor:
  auto`, ссылки `pointer`, ни одного элемента с `cursor: none` — системный
  курсор на всех страницах.
- **Шаг 1:** применён frontend-design skill как линза аудита (запрет цветного
  канта по краю карточки, нумерация только для реальных последовательностей,
  «смелость в одном месте», quality floor для фокуса).
- **Шаг 2:** показан пользователю аудит из 13 пунктов по всем маршрутам;
  согласован план P1+P2+P3 без формы регистрации (структурное изменение —
  отложено по отдельному запросу).
- **Шаг 3 — внесено:**
  - **Нумерация-декор убрана** (skill): нав-карточки `PatientHome` и
    stat-карточки `ClinicQueue` больше не несут `01/02/03/04` — это не
    последовательности. Нумерация roadmap на Landing оставлена (реальные шаги).
  - **TriageMessageCard:** убрана цветная рамка-кант вокруг карточки; вместо неё
    крупный ведущий баннер категории (иконка в круге + метка в mono) в начале
    карточки. Для URGENT_REVIEW — насыщенный red→magenta баннер; проверено на
    скриншоте, что категория считывается мгновенно (сигнал сильнее прежнего
    канта). Тексты `patient_facing_message`/`disclaimer` — verbatim.
  - **Язык поверхностей карточек** (правило пользователя): `glass-card` только
    там, где под карточкой реально градиент/3D — Landing (hero/feature/roadmap)
    и верхняя сводная карточка `PatientHome`. На плоском cream — обычный
    `border-[1.5px] border-line bg-surface shadow-soft`: nav-плитки PatientHome,
    stat-карточки клиники переведены glass→plain.
  - **Пустые состояния:** клиника теперь использует общий `EmptyState` (вынесен
    из `<td>` таблицы), как и пациентка; усилена иерархия caption (основной
    текст вместо мелкого uppercase-muted).
  - **a11y фокус (quality floor):** глобальное `:focus-visible`-кольцо
    (`outline`, не `box-shadow` — намеренно, т.к. outline не обрезается
    `overflow: hidden` родителями) на ссылках/кнопках/табах в `index.css`.
  - **Иерархия/консистентность:** заголовки h1 патиент-подстраниц `text-xl →
    text-2xl`; busy-текст кнопки загрузки `"…"` → `uploadSubmitBusy`
    («Отправляем…»/«Жіберілуде…», добавлены RU+KZ); close-кнопка `Modal` `✕` →
    lucide `<X/>`.
  - **Уплотнение (P3):** карточка `PatientSchedule` — даты в 2-колоночной сетке
    блоков (вместо разреженных строк с большим пустым промежутком), enum статуса
    в mono; заголовок статуса `PatientConsent` выровнен по размеру. Убраны
    мёртвые `data-cursor-interactive` из `Landing.tsx` (остались только в
    неиспользуемом `CustomCursor.tsx`).
- **Не тронуто:** палитра/типографика, `PatientLayout` full-width структура,
  `sw.js`, тексты дисклеймеров/`patient_facing_message`, правила CLAUDE.md,
  никакого PWA. Framer Motion сохранён.
- **Файлы:** `App.tsx`, `index.css`, `components/{EmptyState,Modal,
  TriageMessageCard}.tsx`, `i18n/translations.ts`, `pages/Landing.tsx`,
  `pages/clinic/ClinicQueue.tsx`, `pages/patient/{PatientHome,PatientSchedule,
  PatientConsent,PatientSymptoms,PatientUpload}.tsx`. (`CustomCursor.tsx`
  намеренно НЕ изменён — оставлен в репо без подключения.)
- **Проверка:** `tsc -b` чисто, `vite build` чисто (CSS даже уменьшился
  36.3→33.0 kB — ушли cursor/часть glass); e2e регрессия (auth 20, consent 20,
  data_protection 28, register 14) зелёная. В браузере: подтверждены удаление
  нумерации, plain-карточки, glass только на summary, баннер URGENT, усиленный
  EmptyState, уплотнённый schedule. **Фокус-кольца проверены реальным Tab**
  (не только в CSS): на кнопке RU, logout, sidebar-ссылках и — ключевое —
  на nav-карточке `PatientHome` ВНУТРИ `.cursor-glow` с `overflow: hidden`
  кольцо `:focus-visible` (`2px solid indigo`, offset 2px) рендерится
